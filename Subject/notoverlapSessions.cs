...
namespace Subject
{
    public partial class notoverlapSessions : Form
    {
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "select lec,subj,subcode,tag,grp,session from [Session] where grp like'" + id + "%' ";
            cmd.ExecuteNonQuery();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                dataGridView1.Rows[n].Cells[6].Value = dr["session"].ToString();
            }
            conn.Close();
            // textBox1.Text = " ";
        ...
        {
            this.Hide();
            notOverlapSessionView f1 = new notOverlapSessionView();
            f1.Show();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewRow dr in dataGridView1.Rows)
            {
                if (bool.Parse(dr.Cells[0].Value.ToString()))
                {
                    ...
                    SqlCommand cmd = conn.CreateCommand();
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = " insert into [notOverlapSessions] (lec,subj,subCode,tag,grp) values ( '" + lec + "','" + subj + "','" + subCode + "','" + tag + "','" + grp + "')";
                    cmd.ExecuteNonQuery();
